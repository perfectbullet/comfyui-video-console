  ...
  <div class="app">
    <header>
      <div>
        ...
        <h1>任务管理</h1>
        <p>所有已记录任务的归档状态、详情与生成视频。</p>
      </div>
      <nav>
        <a href="/?mode=director">多模式导演台</a
        ><a href="/?mode=nine-images">九图分镜版</a
        ><a class="active" href="/?mode=tasks">任务管理</a
        ><a href="/?mode=system">系统信息</a>
      </nav>
    </header>
    <main>
      <section class="card summary">
        ...
        <div class="stats">
          <div>
            <strong class="running">{{ running.length }}</strong
            ><span>运行中</span>
          </div>
          <div>
            <strong class="queued">{{ pending.length }}</strong
            ><span>排队中</span>
          </div>
          <div>
            <strong class="completed">{{ completed.length }}</strong
            ><span>已完成</span>
          </div>
          <div>
            <strong class="error">{{ errorCount }}</strong
            ><span>失败</span>
          </div>
        </div>
        <p class="muted">
        ...
        <div class="list-head">
          ...
          <label class="filter"
            >筛选<select v-model="filter">
              ...
              <option value="archiving">正在归档</option>
            ...
            <div class="title">
              ...
              <div class="task-actions">
                ...
                ><button
                  class="delete"
                  type="button"
                  :disabled="deleting.has(t.prompt_id)"
                  @click="deleteTask(t)"
                >
                  {{ deleting.has(t.prompt_id) ? "正在删除…" : "删除任务" }}
                </button>
              </div>
            </div>
            <p class="row-info">
...
function statusClass(status) {
  return successful(status) ? "completed" : failed(status) ? "error" : status;
}
      ...
      response =
        task.status === "queued"
          ? await fetch(`${base}/queue`, {
              ...
              body: JSON.stringify({ delete: [task.prompt_id] }),
            })
...
async function rerunTaskOld(task) {
  ...
  try {
    const response = await fetch(
      `${archiveBase}/api/tasks/${encodeURIComponent(task.prompt_id)}/rerun`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
