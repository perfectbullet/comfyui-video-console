  ...
  <div class="app">
    <header>
      <div>
        ...
        <h1>系统信息</h1>
        <p>
          查看 ComfyUI 运行状态与显存占用，并手动释放显存（卸载模型 +
          清空缓存）。
        </p>
      </div>
      <nav>
        ...
        ><a href="/?mode=tasks">任务管理</a
        ><a class="active" href="/?mode=system">系统信息</a>
      </nav>
    </header>
    <main class="layout">
      <section class="card settings">
        ...
        <div class="row">
          ...
          <label class="check"
            ><input type="checkbox" v-model="auto" /> 每
            {{ interval / 1000 }} 秒自动刷新</label
          >
        </div>
        <p class="muted" v-if="updatedAt" style="margin-top: 10px">
          最近更新：{{ updatedAt }}
        </p>
        <div class="free-block">
          <h3>显存管理</h3>
          <p class="muted">
            工作流跑完后模型会<b>常驻显存</b>以便复用（ComfyUI
            缓存机制）。点击下方按钮可卸载全部模型并清空缓存。
          </p>
          <button class="danger" :disabled="freeing" @click="freeVram">
            {{ freeing ? "释放中…" : "释放显存" }}
          </button>
          <p v-if="freeMsg" :class="['msg', freeOk ? 'ok' : 'err']">
            {{ freeMsg }}
          </p>
          <p class="muted small">
            释放请求发出后，ComfyUI
            需要几秒到几十秒完成模型卸载与缓存回收，显存数字会随后续刷新逐步下降；期间请不要提交新任务。
          </p>
        </div>
      </section>
      <section class="card sys">
        ...
        <template v-else>
          <div class="kv">
            <span>ComfyUI 版本</span><b>{{ sys.comfyui_version || "—" }}</b>
          </div>
          <div class="kv">
            <span>PyTorch</span><b>{{ sys.pytorch_version || "—" }}</b>
          </div>
          <div class="kv">
            <span>Python</span
            ><b>{{ (sys.python_version || "—").split(" (")[0] }}</b>
          </div>
          <div class="kv">
            <span>操作系统</span
            ><b>{{ sys.os === "linux" ? "Linux" : sys.os }}</b>
          </div>
          <div class="kv">
            <span>任务队列</span
            ><b :class="queueBusy ? 'warn' : 'ok-text'"
              >运行中 {{ queue.running }} · 排队 {{ queue.pending }}</b
            >
          </div>
          <div class="mem">
            <div class="mem-head">
              <span>内存（RAM）</span
              ><span>{{ gb(ramUsed) }} / {{ gb(sys.ram_total) }} GB</span>
            </div>
            <div class="bar">
              <i
                :style="{ width: pct(ramUsed, sys.ram_total) + '%' }"
                class="ram"
              />
            </div>
          </div>
      ...
      <section class="card dev">
        ...
        <div v-for="d in devices" :key="d.index" class="device">
          <div class="dev-head">
            <b>{{ d.name }}</b
            ><span class="pill">{{ d.type }} · #{{ d.index }}</span>
          </div>
          <div class="mem">
            <div class="mem-head">
              <span>显存（VRAM）</span
              ><span
                >{{ gb(vramUsed(d)) }} / {{ gb(d.vram_total) }} GB（空闲
                {{ gb(d.vram_free) }} GB）</span
              >
            </div>
            <div class="bar">
              <i
                :style="{ width: pct(vramUsed(d), d.vram_total) + '%' }"
                class="vram"
              />
            </div>
          </div>
          <p class="muted small">
            占用为整卡口径（含本机其他进程）；释放显存只作用于 ComfyUI 进程。
          </p>
        </div>
      </section>
    </main>
...
const ramUsed = computed(() => {
  const s = sys.value;
  return s.ram_total && s.ram_free != null ? s.ram_total - s.ram_free : 0;
});
const queueBusy = computed(
  () => queue.value.running > 0 || queue.value.pending > 0,
);
const vramUsed = (d) =>
  d.vram_total && d.vram_free != null ? d.vram_total - d.vram_free : 0;
...
async function refresh() {
  ...
  try {
    ...
    stats.value = null;
    updatedAt.value = "";
    freeMsg.value = `状态获取失败：${e.message}`;
    freeOk.value = false;
  } finally {
    loading.value = false;
  }
...
async function freeVram() {
  ...
  if (
    queueBusy.value &&
    !confirm(
      `当前有 ${queue.value.running} 个任务运行中、${queue.value.pending} 个排队中。\n释放显存会在任务间隙执行模型卸载，可能影响后续任务速度。确定继续吗？`,
    )
  )
    return;
  freeing.value = true;
  ...
  freeOk.value = false;
  try {
    const r = await fetch(`${server.value}/free`, {
      method: "POST",
    ...
    freeOk.value = true;
    freeMsg.value =
      "释放请求已发送（接口立即返回）。模型卸载 + 缓存回收是异步进行的，请在下方观察显存数字逐步下降。";
    setTimeout(refresh, 2000);
    setTimeout(refresh, 6000);
    setTimeout(refresh, 15000);
  } catch (e) {
    freeMsg.value = `释放请求失败：${e.message}`;
    freeOk.value = false;
  } finally {
    freeing.value = false;
  }
